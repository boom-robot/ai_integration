...
class HandTriggerResult:
    ...
    def find_command_detecting(self, multi_hand_lms):
        total_fingers = []
        fingers = []
        fingers_stop = []
    ...
    def find_stop(self, multi_hand_lms, time_to_stop=4,):
        fingers = []
        for hand_lms in multi_hand_lms:
    ...
    def process_hands(self, cv_img: MatLike, draw=True):
        ...
        self.results = self.hands.process(img_RGB)
          ...
          # Initialize a list to store detected hand landmarks

        if not self._has_results():
            self.flag_start_stop = False
            self.time_start = 0
            self.duration_time = 0
            return cv_img, self.results.multi_hand_landmarks 
        if draw:
            for hand_lms in self.results.multi_hand_landmarks:
